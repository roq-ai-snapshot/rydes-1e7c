...
import { getTravellingAllowanceById, updateTravellingAllowanceById } from 'apiSdk/travelling-allowances';
...
function TravellingAllowanceEditPage() {
  ...
  const handleSubmit = async (values: TravellingAllowanceInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateTravellingAllowanceById(id, values);
      mutate(updated);
      resetForm();
    } catch (error) {
      setFormError(error);
